...
function populateUsersTable(users) {
    ...
    allUsers = users.map(user => ({
        ...
        role: user.role,
        gender: user.gender,
        grade: user.grade || user.year_level || '',
    ...
    // Keep filter dropdowns synced with current user data.
    populateUsersFilterDropdowns();
...
function setupFilterEvents() {
    ...
    if (gradeFilter) {
        gradeFilter.addEventListener('change', () => {
            populateSectionFilterOptions(gradeFilter.value || 'all');
            filterAndPaginateUsers();
        });
    }
    if (sectionFilter) sectionFilter.addEventListener('change', filterAndPaginateUsers);
...
function populateUsersFilterDropdowns() {
    const gradeFilter = document.getElementById('gradeFilter');
    const sectionFilter = document.getElementById('sectionFilter');
    ...
    if (gradeFilter) {
        ...
        const grades = Array.from(
            new Set(allUsers.map(user => String(user.grade || '').trim()).filter(Boolean))
        ...
        gradeFilter.value = grades.includes(selectedGrade) ? selectedGrade : 'all';
    }

    populateSectionFilterOptions((gradeFilter && gradeFilter.value) ? gradeFilter.value : 'all');
}

function populateSectionFilterOptions(selectedGrade = 'all') {
    const sectionFilter = document.getElementById('sectionFilter');
    ...
    const sections = Array.from(
        new Set(
            allUsers
                .filter(user => selectedGrade === 'all' || String(user.grade || '') === String(selectedGrade))
    ...
    sectionFilter.value = sections.includes(selectedSection) ? selectedSection : 'all';
}
...
function filterAndPaginateUsers() {
    ...
    const filteredUsers = allUsers.filter(user => {
        ...
        const matchesGrade = gradeFilter === 'all' || String(user.grade || '') === gradeFilter;
        const matchesSection = sectionFilter === 'all' || String(user.section || '') === sectionFilter;
        ...
        return matchesRole && matchesStatus && matchesGrade && matchesSection && matchesGender && matchesSearch;
    });
