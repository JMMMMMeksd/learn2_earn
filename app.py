...
def get_current_quarter():
    ...
    try:
        ...
        # If multiple rows are marked active (school-year-level flag),
        # still choose the quarter matching today's date.
        explicit_active = [q for q in quarters_result.data if str(q.get('status', '')).lower() == 'active']
        if explicit_active:
            now_date = now_str.split('T')[0]
            for quarter in explicit_active:
                ...
                if start_date_part <= now_date <= end_date_part:
                    ...
                    app.logger.info(
                        f"Found active quarter by status+date: {quarter.get('quarter_name')} "
                        f"({quarter.get('school_year')})"
                    )
                    return quarter
            ...
            app.logger.info(
                f"Found active quarter by status fallback: {active_quarter.get('quarter_name')} "
...
@app.route('/api/school-year-status/<string:school_year>')
def check_school_year_status(school_year):
    """Check if a school year has ended (4th quarter passed)"""
    try:
        ...
        result = supabase.table('quarters')\
            .select('quarter_name, end_date, status')\
            .eq('school_year', school_year)\
            .order('start_date')\
            .execute()
        
        quarters = result.data or []
        if not quarters:
            return jsonify({'success': True, 'has_ended': False, 'message': 'No quarters found'}), 200
        
        # Helper to parse quarter end date
        def parse_quarter_end_date(raw_value):
        ...
        fourth_quarter = next(
            (q for q in quarters
             if any(token in str(q.get('quarter_name', '')).lower()
                   for token in ('4th', 'fourth', 'quarter 4', 'q4'))),
            None
        )
        
        if fourth_quarter is None:
            quarter_candidates = [(q, parse_quarter_end_date(q.get('end_date'))) for q in quarters]
            quarter_candidates = [item for item in quarter_candidates if item[1] is not None]
            if quarter_candidates:
                fourth_quarter = max(quarter_candidates, key=lambda item: item[1])[0]
            else:
                fourth_quarter = quarters[-1]
        
        # Check if 4th quarter has ended
        quarter_end_date = parse_quarter_end_date(fourth_quarter.get('end_date'))
        if quarter_end_date is None:
            return jsonify({'success': True, 'has_ended': False}), 200
        
        today = datetime.now(philippines_tz).date()
        has_ended = today > quarter_end_date
        
        return jsonify({
            'success': True,
            'has_ended': has_ended,
            'fourth_quarter_end': quarter_end_date.isoformat(),
            'today': today.isoformat()
        }), 200
        
    except Exception as e:
        logger.error(f"Error checking school year status: {str(e)}")
        return jsonify({'success': False, 'message': str(e), 'has_ended': False}), 500
...
def execute_school_year_transition():
    ...
    try:
        ...
        try:
            ...
            # Update quarter lifecycle statuses for transition state tracking.
            # Keep a single active school year marker.
            supabase.table('quarters')\
                .update({'status': 'ended'})\
                .neq('school_year', next_school_year)\
                .execute()

            supabase.table('quarters')\
                .update({'status': 'active'})\
                .eq('school_year', next_school_year)\
                .execute()
...
def school_year_transition_rollback():
    ...
    try:
        ...
        try:
            ...
            # Rollback quarter lifecycle statuses.
            # The rolled-back school year becomes active again and others become ended.
            supabase.table('quarters')\
                .update({'status': 'ended'})\
                .neq('school_year', school_year_to_rollback)\
                .execute()
...
def create_teacher_quiz():
    ...
    try:
        ...
        # Backward/defensive handling: accept classroom key format "grade|section"
        # if grade_level/section are not explicitly provided by the client.
        raw_grade_level = data.get('grade_level')
        raw_section = data.get('section')
        grade_level = str(raw_grade_level).strip() if raw_grade_level is not None else ''
        section = str(raw_section).strip() if raw_section is not None else ''
        if not grade_level or not section:
            classroom_key = (
                data.get('classroom')
                or data.get('classroom_key')
                or data.get('assign_classroom')
                or ''
            )
            if isinstance(classroom_key, str) and '|' in classroom_key:
                parsed_grade, parsed_section = classroom_key.split('|', 1)
                grade_level = grade_level or parsed_grade.strip()
                section = section or parsed_section.strip()
        ...
        # Prefer explicit school year from client, otherwise use server-side current value
        resolved_school_year = data.get('school_year') or get_current_school_year()
        quiz_record = {
            'teacher_id': teacher_id,
        ...
        if is_update:
            ...
            try:
                ...
                # Backward compatibility: if DB schema doesn't have school_year yet, retry without it
                msg = str(update_err).lower()
                if 'school_year' in msg:
                    fallback_record = dict(quiz_record)
                    fallback_record.pop('school_year', None)
                    result = supabase.table('teacher_quizzes')\
                        .update(fallback_record)\
                        .eq('id', quiz_id)\
                        .eq('teacher_id', teacher_id)\
                        .execute()
                else:
            ...
            try:
                ...
                if 'school_year' in msg:
                    fallback_record = dict(quiz_record)
                    fallback_record.pop('school_year', None)
                    result = supabase.table('teacher_quizzes').insert(fallback_record).execute()
                else:
                    raise
...
def get_my_quizzes():
    ...
    try:
        ...
        if school_year_filter:
            try:
                query = query.eq('school_year', school_year_filter)
            except Exception:
                # If schema doesn't include school_year, ignore filter
                pass

        if quarter_filter:
            try:
                query = query.eq('quarter', quarter_filter)
            except Exception:
                pass
        ...
        for quiz in result.data:
            ...
            # Get total ACTIVE students in the class (exclude archived/inactive)
...
def get_teacher_info():
    """Get teacher information including name and active class assignments across school years."""
    try:
        teacher_id = session.get('user_id')
        ...
        # Get teacher's active class assignments across school years.
        # Backward compatible: if status/school_year columns aren't available yet, fall back.
        try:
            assignments = supabase.table('teacher_class_assignments')\
                .select('grade_level, section, school_year, status')\
                .eq('teacher_id', teacher_id)\
                .eq('status', 'active')\
                .execute()
        except Exception as assignment_err:
            logger.warning(f"teacher_class_assignments query failed with status filter: {assignment_err}")
...
def get_student_quizzes():
    ...
    try:
        ...
        student = supabase.table('user_info')\
            .select('first_name', 'last_name', 'year_level', 'section', 'status')\
            .eq('id', student_id)\
            .execute()
        ...
        if student_status not in ['Active', 'active']:
            return jsonify({'error': 'This account is no longer active and cannot access quizzes'}), 403
...
def send_quiz_reminder():
    ...
    try:
        ...
        if remind_all:
            # Get all ACTIVE students in the class who haven't submitted
            class_students = supabase.table('user_info')\
                .select('id, first_name, last_name')\
                .eq('role', 'Student')\
                .eq('year_level', quiz_data['grade_level'])\
...
@app.route('/api/school-years/<school_year>', methods=['DELETE'])
def api_delete_school_year(school_year):
    """Delete a school year and all its quarters"""
    try:
        # Check if user is admin
        if session.get('role') != 'Admin':
            return jsonify({'success': False, 'message': 'Only admins can delete school years'}), 403
        
        # Delete all quarters for this school year
        delete_result = supabase.table('quarters') \
            .delete() \
            .eq('school_year', school_year) \
            .execute()
        
        app.logger.info(f"Deleted school year {school_year} and all its quarters")
        return jsonify({'success': True, 'message': f'School year {school_year} and all its quarters have been deleted'})
    except Exception as e:
        app.logger.error(f"Error deleting school year {school_year}: {str(e)}")
        return jsonify({'success': False, 'message': str(e)}), 500
...
def submit_student_quiz():
    ...
    try:
        ...
        student_status = student_data.get('status', 'Active')
        if student_status not in ['Active', 'active']:
            return jsonify({'error': 'This account is no longer active and cannot submit quizzes'}), 403
...
def upload_file_to_supabase(file, task_id, teacher_id):
    """Upload a file to Supabase Storage and return file metadata as JSON-serializable dict"""
    try:
        ...
        # Return as JSON-serializable object (dict) so it can be stored in JSON/JSONB
        # without being double-encoded into an escaped string.
        attachment_json = {
            "file_url": public_url,
            "original_filename": original_filename,
            "file_type": file_type,
...
def create_activity():
    ...
    try:
        ...
        # Store as list[dict] so DB keeps real JSON (not escaped strings).
        file_attachments = []
            ...
            for idx, file in enumerate(files):
                if file and file.filename != '':
                    try:
                        ...
                        file_obj = upload_file_to_supabase(file, temp_task_id, teacher_id)
                        if file_obj:
        ...
        # Store links as list[dict] to avoid double-encoding
        links_data = links if isinstance(links, list) else []
        ...
        if assignments:
            ...
            try:
                # Get teacher information for notification title
                teacher_info = supabase.table('user_info').select('first_name, last_name, gender').eq('id', teacher_id).limit(1).execute()
                
                if teacher_info.data and len(teacher_info.data) > 0:
                    teacher_data = teacher_info.data[0]
                    teacher_name = get_teacher_name_with_prefix(teacher_data)
                else:
                    teacher_name = "Your Teacher"
                
                # Extract task_ids from inserted result
                task_ids = []
                ...
                if due_date:
                    try:
                        ...
                        due_date_display = f" - Due: {due_dt.strftime('%B %d, %Y')}"
                    except:
                        due_date_display = f" - Due: {due_date}"
                
                # Create one notification per unique task_id/student combination
                if task_ids and len(task_ids) > 0:
                    # Group by task_id
                    for idx, task_id in enumerate(task_ids):
                        if task_id and idx < len(students):
                            student = students[idx]
                            notifications_to_create.append({
                                'user_id': student['id'],
                                'sender_id': teacher_id,
                                'title': f"{teacher_name} assigned a new activity",
                                'message': f"{teacher_name} assigned a new activity: {task_title}{due_date_display}",
                                'task_id': task_id,
                                'notif_type': 'Task',
                                'status': 'Unread'
                            })
                ...
                if notifications_to_create:
                    ...
                    logger.info(f"Created {len(notifications_to_create)} notifications for activity {activity_group_id}")
                
            except Exception as notif_err:
                logger.error(f"Error creating notifications: {str(notif_err)}")
...
def update_activity():
    ...
    try:
        ...
        # Process new file uploads - returns dict objects
            ...
            for idx, file in enumerate(files):
                if file and file.filename != '':
                    try:
                        ...
                        file_obj = upload_file_to_supabase(file, temp_task_id, teacher_id)
                        if file_obj:
                            new_file_attachments.append(file_obj)
                    except Exception as file_err:
                        logger.error(f"Error uploading file {idx}: {str(file_err)}")
                        # Continue with other files, don't fail the entire update
        
        # Keep existing files as JSON objects where possible (avoid stringifying).
        existing_files_objs = []
        for file_obj in existing_files:
            try:
                if isinstance(file_obj, dict):
                    ...
                    existing_files_objs.append(attachment)
                elif isinstance(file_obj, str):
                    # Legacy JSON string: try to parse into object; otherwise keep as-is.
                    try:
                        parsed = json.loads(file_obj)
                        existing_files_objs.append(parsed if isinstance(parsed, dict) else file_obj)
                    except Exception:
                        existing_files_objs.append(file_obj)
            except Exception as err:
                logger.warning(f"Error processing existing file: {str(err)}")
                # Skip this file and continue
        
        # Combine existing and new files
        all_file_attachments = existing_files_objs + new_file_attachments
        ...
        update_data = {
            ...
            'priority': priority,
            'template': template,
            'attachments': all_file_attachments if all_file_attachments else None,
            'links': links_data if links_data else None
        }
